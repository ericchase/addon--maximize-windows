...
document.getElementById('maximize-all-windows').addEventListener('click', function () {
...
for (const key in checkboxTitles) {
	const item = _g.parseHTML(`<label class="checkbox"><input type="checkbox" id="${key}" ${settings[key] ? 'checked' : ''}>${checkboxTitles[key]}</label>`);
