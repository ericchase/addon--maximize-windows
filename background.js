...
async function runtime$onStartup() {
	...
	const { 'last-focused-tab-info': last_focused_tab_info } = await cache.get(['last-focused-tab-info']);
...
function runtime$onMessage(message, sender, sendResponse) {
	...
	(async function () {
		const { enabled: enabled } = await cache.get(['enabled']);
...
async function windows$onCreated(window) {
	...
	const { enabled: enabled, 'maximize-window-on-creation': maximize_window_on_creation } = await cache.get(['enabled', 'maximize-window-on-creation']);
...
async function action$onClicked(tab) {
	...
	const { enabled: enabled, 'ignore-minimized-windows': re_minimize_windows } = await cache.get(['enabled', 'ignore-minimized-windows']);
...
async function maximize(window) {
	...
	const { 'ignore-minimized-windows': ignore_minimized_windows } = await cache.get(['ignore-minimized-windows']);
...
async function createContextMenuItems() {
	...
	} = await cache.get(['enabled', 'maximize-window-on-creation', 'ignore-minimized-windows']);
